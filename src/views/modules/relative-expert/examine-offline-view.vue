<template>
  <div class="mod-policy">
    <h2 style="border-bottom: 1px solid #ccc;padding-bottom: 20px;margin-bottom: 50px">审核查看<el-button type="primary" style="margin-left:20px" size="small" @click="$router.push({ name: 'relative-expert-view',query:{id:$route.query.id} })">查看在线内容</el-button></h2>
    <el-form label-position="left" label-width="120px" :model="dataForm" ref="dataForm">
      <div>
        <el-form label-position="left" label-width="100px"  :model="dataForm" ref="dataForm" style="border:1px solid #ccc;padding-left:50px;padding-top:25px;margin-bottom:30px">
          <el-form-item label="解读ID" prop="expertId"  style="display:inline-block">
            <el-input v-model="dataForm.expertId" :disabled="true" style="width:250px"></el-input>
          </el-form-item>
...
export default {
  ...
  mounted(){
    if( this.dataForm.id!=undefined) {
      this.$http({
        url: this.$http.adornUrl(`/biz/trpolicyrelativeexpert/applyDownlineNoPassInfo`),
        method: 'get',
        params: this.$http.adornParams({recordId:this.dataForm.id})
      }).then(({data}) => {
