<template>
  <div class="mod-policy">
    <el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()">
      <el-form-item>
        <el-button type="warning" v-if="isAuth('biz:trrisk:save')" @click="$router.push({ name: 'risk-tips-add-or-update'})">新增</el-button>
      </el-form-item>
      <el-form-item>
      ...
      <el-form-item>
        <el-input v-model="dataForm.lecturerName" placeholder="作者" clearable></el-input>
      </el-form-item>
      <el-form-item>
        <el-select
    ...
    <el-table
      ...
      <el-table-column
        ...
        label="状态">
        <template slot-scope="scope">
          <el-button v-show="scope.row.ifShow==0" v-if="isAuth('biz:trrisk:online')" type="text" size="small" @click="hideToShow(scope.row.id)">隐藏</el-button>
          <el-button v-show="scope.row.ifShow==1" v-if="isAuth('biz:trrisk:hide')" type="text" size="small" @click="showToHide(scope.row.id)">在线</el-button>
        </template>
      </el-table-column>
      <el-table-column
        fixed="right"
        ...
        label="操作">
        <template slot-scope="scope">
          <el-button type="text" size="small" @click="$router.push({ name: 'risk-tips-record',query:{id:scope.row.id,title:scope.row.title} })">操作记录</el-button>
          <el-button type="text" size="small" v-if="isAuth('biz:trrisk:update')" @click="$router.push({ name: 'risk-tips-add-or-update',query:{id:scope.row.id} })">编辑</el-button>
          <el-button type="text" size="small" v-if="isAuth('biz:trrisk:info')" @click="$router.push({ name: 'risk-tips-view',query:{id:scope.row.id} })">查看</el-button>
          <el-button type="text" size="small" v-if="isAuth('biz:trrisk:delete')" @click="deleteHandle(scope.row.id)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
  ...
  export default {
    ...
    methods: {
      ...
      sortUpdate(index,id,sort){
        this.$http({
          url: this.$http.adornUrl('/biz/trrisk/updateSort'),
          method: 'post',
      ...
      // 删除
      deleteHandle (id) {
        this.confirmFn('提示','您确定要删除该风险提示吗？','确定','/biz/trrisk/delete/',id)
      },
      // 隐藏按钮--》在线
      hideToShow (id) {
        this.confirmFn('提示','您确定要在线吗？','确定','/biz/trrisk/online/',id)
      },
      // 在线按钮--》隐藏
      showToHide (id) {
        this.confirmFn('提示','您确定要隐藏吗？','确定','/biz/trrisk/hide/',id)
      },
      // 获取数据列表
      getDataList () {
        ...
        this.$http({
          url: this.$http.adornUrl('/biz/trrisk/list'),
          method: 'get',
          params: this.$http.adornParams({
