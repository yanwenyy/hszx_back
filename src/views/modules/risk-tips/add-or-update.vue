  ...
  <div class="mod-policy">
    ...
    <el-form label-position="left" label-width="120px" :model="dataForm" :rules="dataRule" ref="dataForm">
      ...
      <el-form-item label="内容" prop="content">
        <template>
          <UEditor :contentUrl='"/biz/trrisk/info/"' :key="'editor_risk-tips'" :val="dataForm.id" :url="'/biz/trrisk/info/'" :id='"editor_risk-tips"':index="0" :econtent="dataForm.content"  @func="editorContent" ></UEditor>
        </template>
      </el-form-item>
  ...
  methods:{
    ...
    dataFormSubmit () {
      this.$refs['dataForm'].validate((valid) => {
          if (valid) {
          this.$http({
            url: this.$http.adornUrl(`/biz/trrisk/${!this.dataForm.id ? 'save' : 'update'}`),
            method: 'post',
