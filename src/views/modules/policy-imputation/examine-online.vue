...
<!--      <el-form-item label="政策状态" prop="policyStatusName">
      ...
      <el-form-item style="text-align: center;">
        <el-button type="primary" @click="closePage();$router.push({ name: 'policy-imputation-examine-edit',query:{id:dataForm.id,editType:'online'}})">编辑</el-button>
    ...
    mounted(){
      if( this.dataForm.id!=undefined) {
        this.$http({
          ...
          this.dataForm.propertyName=data.data.propertyName
    ...
    methods:{
      ...
      fail () {
        this.$prompt('请输入未通过原因，最少5个字最多输入100字', '审核未通过', {
          ...
          this.$http({
            url: this.$http.adornUrl('/biz/trpolicy/onlineAuditNoPass'),
            method: 'GET',
      ...
      dataFormSubmit () {
        this.$confirm(`您确定审核通过吗？`, ``, {
          ...
          type: 'warning'
              ...
              this.$http({
                url: this.$http.adornUrl(`/biz/trpolicy/onlineAuditPassNoChange/${this.dataForm.id}`),
                method: 'get'
              }).then(({data}) => {
                if (data && data.code == 200) {
                  this.$message({
