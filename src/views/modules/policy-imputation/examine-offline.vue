<template>
  <div class="mod-policy">
    <h2 style="border-bottom: 1px solid #ccc;padding-bottom: 20px;margin-bottom: 50px">审核<el-button type="primary" style="margin-left:20px" size="small" @click="$router.push({ name: 'policy-imputation-view',query:{id:$route.query.id} })">查看在线内容</el-button></h2>
    <div>
      <el-form label-position="left" label-width="100px"  :model="dataForm" ref="dataForm" style="border:1px solid #ccc;padding-left:50px;padding-top:25px;margin-bottom:30px">
        ...
        <el-form-item label="审核人" prop="applyIdName"  style="display:inline-block;margin-left:50px">
        ...
        <el-form-item label="审核时间" prop="applyTime"style="display:inline-block;margin-left:50px">
  ...
  methods:{
    ...
    fail () {
      this.$prompt('请输入未通过原因，最少5个字最多输入100字', '审核未通过', {
        ...
        this.$http({
          url: this.$http.adornUrl('/biz/trpolicy/offlineAuditNoPass'),
          method: 'GET',
    ...
    dataFormSubmit () {
      this.$confirm(`您确定审核通过吗？`, ``, {
        ...
        this.$http({
          url: this.$http.adornUrl(`/biz/trpolicy/offlineAuditPass/${this.dataForm.id}`),
          method: 'get',
