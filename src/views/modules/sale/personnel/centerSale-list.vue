    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <el-button type="text" size="small" v-if="isAuth('biz:trpolicyoriginal:update')"  @click="$router.push({ name: 'centerSale-add-or-update',query:{id:scope.row.uuid} })">编辑</el-button>
          <el-button type="text" size="small" v-if="isAuth('biz:trpolicyoriginal:info')" @click="$router.push({ name: 'centerSale-view',query:{id:scope.row.uuid} })">查看</el-button>
  ...
  export default {
    ...
    methods: {
        ...
        this.$refs['diaLog'].validate((valid) => {
          ...
          this.$http({
            ...
            if(data.code==200){
              ...
              this.$router.push({ name: 'centerSale-add-or-update',query:{'phone': this.dataForm.searchPhone,'username':datas.username,'realname':datas.realname} })
            }else{
      ...
      getDataList () {
        ...
        this.$http({
          url: this.$http.adornUrl('/biz/orgTbUser/centerUserListOfPaging'),
          method: 'get',
