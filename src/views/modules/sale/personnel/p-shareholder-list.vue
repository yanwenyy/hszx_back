  ...
  <div class="mod-policy">
    ...
    <el-table
      ...
      <el-table-column
        ...
        label="操作">
        <template slot-scope="scope">
          <el-button type="text" size="small" v-if="isAuth('biz:trpolicyoriginal:update')"  @click="$router.push({ name: 'p-shareholder-add-or-update',query:{id:scope.row.uuid} })">编辑</el-button>
          <el-button type="text" size="small" v-if="isAuth('biz:trpolicyoriginal:info')" @click="$router.push({ name: 'p-shareholder-view',query:{id:scope.row.uuid} })">查看</el-button>
    ...
    methods: {
          ...
          this.$http({
            ...
            method: 'GET',
              ...
              if(data.code==200){
                ...
                this.$router.push({ name: 'p-shareholder-add-or-update',query:{'phone': this.dataForm.searchPhone,'username':datas.username,'realname':datas.realname} })
              }else{
      ...
      deleteHandle (id,status) {
        ...
        this.$confirm(`您确定要删除${policyId.length > 1 ? '[id=' + policyId.join(',') + ']':'该'}数据吗？`, ``, {
          ...
          this.$http({
            url: this.$http.adornUrl('/biz/orgTbUser/deleteShareUser/'+id),
            method: 'get'
          }).then(({data}) => {
      ...
      getDataList () {
        ...
        this.$http({
          url: this.$http.adornUrl('/biz/orgTbUser/shareUserListOfPaging'),
          method: 'get',
