      ...
      <div class="city-view-group">
        <div class="city-view-box">
          ...
          <el-form-item label="选择角色">
            <el-radio :disabled="saleAdminNum>3" v-model="dataForm.nowRole" label="5">股东管理员</el-radio>
            <el-radio :disabled="saleNum>17" v-model="dataForm.nowRole" label="4">股东销售</el-radio>
  ...
  export default {
    ...
    activated(){
      ...
      if( this.dataForm.id!=undefined) {
        this.$http({
          url: this.$http.adornUrl(`/biz/orgTbUser/infoAgentUser/${this.dataForm.id}`),
          method: 'get',
          ...
          this.dataForm.agencyId=datas.agencyId;
    ...
    methods:{
      ...
      getJxs(){
        if(this.dataForm.shareholderId!==''){
          ...
          this.$http({
            ...
            method: 'GET',
            params: this.$http.adornParams({
              pid: this.dataForm.shareholderId
            })
          }).then(({data}) => {
      ...
      //经销商列表选择
      getGdNum(val){
        this.saleAdminNum = val ? this.jxsList.find(ele => ele.id === val).saleAdminNum : '';
        this.saleNum = val ? this.jxsList.find(ele => ele.id === val).saleNum : '';
      },
      //提交表单
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          this.$http({
            url: this.$http.adornUrl(`/biz/orgTbUser/${!this.dataForm.id ? 'saveAgentUser' : 'updateAgent'}`),
            method: 'post',
