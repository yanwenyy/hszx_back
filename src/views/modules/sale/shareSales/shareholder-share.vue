<template>
  <div class="mod-policy">
    <el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()">
      ...
      <el-form-item>
        <el-input v-model="dataForm.orgName" placeholder="股东机构名称" clearable></el-input>
      </el-form-item>
      <el-form-item>
        <el-select
    ...
    <el-table
      ...
      <el-table-column
        ...
        label="操作">
        <template slot-scope="scope">
          <el-button type="text" size="small" v-if="isAuth('biz:trpolicyoriginal:update')"  @click="openDialog(2,'产品直销分成设置',scope.row.id),cardForm.saleName=scope.row.name">修改分成</el-button>
          <el-button type="text" size="small" v-if="isAuth('biz:trpolicyoriginal:info')" @click="openDialog(3,'分成修改记录',scope.row.id),recordForm=scope.row">查看记录</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
    ...
    <el-dialog class="bigDialog" :title="dialogTitle" :visible.sync="dialogFormVisible">
      ...
      <div v-show="dialogType==2">
        <div class="city-view-title">股东机构名称：{{cardForm.saleName}}</div>
        <div>
      ...
      <div v-show="dialogType==3">
        <div>
          <div class="fc-list">
            <span class="record-name">机构名称:</span>
            <el-input class="record-input" v-model="recordForm.name" disabled></el-input>
          </div>
          <div class="fc-list">
    ...
    <el-dialog title="分成设置" :visible.sync="editShareVisible">
      <div class="city-view-title">股东机构名称：{{editShareTitle}}</div>
  ...
  export default {
    ...
    methods: {
      ...
      getDataList () {
        ...
        this.$http({
          url: this.$http.adornUrl('/biz/cardaccount/orgList'),
          method: 'GET',
